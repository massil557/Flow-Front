...
import { LayoutGrid, Activity, Bell, Search, User, ShieldCheck, LogOut, Map, ServerCog, Settings } from "lucide-react";
...
const MobileNavItem = ({ icon: Icon, label, to, active }) => (
  <Link
    ...
    className={`flex flex-col items-center justify-center gap-1 flex-1 py-2 transition-all duration-200
      ${active ? 'text-[#17203f] dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}
  >
...
export default function MainLayout() {
  ...
  const navItems = isAutomatician ? [
    { icon: LayoutGrid, label: t('nav.dashboard'),   to: '/mainlayout/dashboard' },
    { icon: Activity,   label: t('nav.analytics'),   to: '/mainlayout/analytics' },
    { icon: Bell,       label: t('nav.alerts'),       to: '/mainlayout/alerts'    },
    { icon: Map,        label: t('nav.plantmap'),     to: '/mainlayout/plantmap'  },
  ] : [];

  return (
    <div className="flex min-h-screen bg-[#F8F9FB] dark:bg-[#0f172a] overflow-x-hidden">

      {/* ── SIDEBAR ─────────────────────────────────────────────────────────── */}
      <aside className="hidden lg:flex w-52 bg-[#17203f] dark:bg-[#0a0f1e] flex-col fixed left-0 top-0 h-screen z-20 shadow-xl">
        ...
        <nav className="flex flex-col gap-0.5 px-3 flex-1 overflow-y-auto">
          ...
          <div className="px-4 pt-4 pb-1">
            <p className="text-[10px] text-white/30 uppercase tracking-widest font-bold">{t('nav.settings')}</p>
          </div>
          <SidebarItem
            icon={Settings}
            label={t('nav.settings')}
            to="/mainlayout/settings"
            active={isActive("/mainlayout/settings")}
          />
          ...
          {isAdmin && (
            <>
              <div className="px-4 pt-5 pb-1">
                <p className="text-[10px] text-white/30 uppercase tracking-widest font-bold">{t('nav.admin')}</p>
              </div>
              <SidebarItem
                icon={ShieldCheck}
                label={t('nav.users')}
      ...
      <div className="flex-1 flex flex-col lg:ml-52 h-screen overflow-hidden">
        ...
        <header className="hidden lg:flex items-center justify-between h-16 bg-white dark:bg-[#1e293b] border-b border-slate-100 dark:border-slate-700 px-6 sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input
                ...
                className="bg-slate-50 dark:bg-slate-800 rounded-xl pl-9 pr-4 py-2 text-sm w-64 outline-none focus:ring-2 focus:ring-[#17203f]/20 border border-slate-200 dark:border-slate-600 transition-all font-medium dark:text-white"
              />
            </div>
            <Link
              to="/mainlayout/profile"
        ...
        <header className="lg:hidden flex items-center justify-between px-4 py-4 bg-white dark:bg-[#1e293b] border-b border-slate-100 dark:border-slate-700 sticky top-0 z-10 shadow-sm">
          <img src={cevitalLogo} alt="Cevital" className="h-7 object-contain" />
          <div className="flex items-center gap-2">
            <button onClick={() => setSearchOpen(!searchOpen)} className="p-2.5 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl transition-colors">
        ...
        {searchOpen && (
          <div className="lg:hidden flex items-center gap-3 px-4 py-3 bg-white dark:bg-[#1e293b] border-b border-slate-100 dark:border-slate-700">
            <Search className="text-slate-400 shrink-0" size={15} />
            <input type="text" placeholder={t('common.search')} autoFocus className="flex-1 bg-transparent text-sm outline-none font-medium dark:text-white" />
          </div>
        )}
      ...
      {/* ── MOBILE BOTTOM NAV ───────────────────────────────────────────────── */}
      <nav className="fixed bottom-0 left-0 right-0 lg:hidden bg-white dark:bg-[#1e293b] border-t border-slate-200 dark:border-slate-700 z-40 shadow-lg">
        <div className="flex items-stretch h-16 px-2">
          {navItems.map(item => (
            item.to === '/mainlayout/alerts' ? (
              <Link
                key={item.to}
                to={item.to}
                className="relative flex flex-col items-center justify-center gap-1 flex-1 py-2 transition-all duration-200"
              >
                <Bell size={22} strokeWidth={isActive(item.to) ? 2.5 : 1.8} className={isActive(item.to) ? 'text-[#17203f] dark:text-white' : 'text-slate-400 dark:text-slate-500'} />
                {activeCount > 0 && (
                  <div className="absolute -top-1 right-1/4">
                    <div className="bg-white/90 dark:bg-slate-700 backdrop-blur-sm rounded-md px-1.5 py-0.5 shadow-sm border border-slate-200 dark:border-slate-600">
          ...
          {isAdmin && (
            <>
              <MobileNavItem icon={ShieldCheck} label={t('nav.admin_short')}   to="/mainlayout/admin"         active={isActive("/mainlayout/admin")} />
              <MobileNavItem icon={ServerCog}   label={t('nav.server_short')}  to="/mainlayout/server-status" active={isActive("/mainlayout/server-status")} />
            </>
          )}
          <MobileNavItem icon={Settings} label={t('nav.settings')} to="/mainlayout/settings" active={isActive("/mainlayout/settings")} />
          <MobileNavItem icon={User} label={t('nav.profile')} to="/mainlayout/profile" active={isActive("/mainlayout/profile")} />
        </div>
      </nav>
