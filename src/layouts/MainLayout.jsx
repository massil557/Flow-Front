...
// Uses useAuth() to get real user data and wire the Logout button.
// The hardcoded  const user = 'automatician'  is replaced by context.

import { Outlet, Link, useLocation, useNavigate } from "react-router-dom";
import { LayoutGrid, Activity, Bell, Settings, Search, User } from "lucide-react";
...
const SidebarItem = ({ to = '/', label, active }) => (
  <Link
    to={to}
    className={`flex items-center px-4 py-3 w-full rounded-lg relative transition-all duration-300 ease-in-out
      ${active ? "bg-white/10 text-white border-r-4 border-purple-500" : "text-gray-300 hover:bg-white/5 hover:text-white"}`}
  >
...
const MobileNavItem = ({ icon: Icon, label, to, active }) => (
  <Link
    ...
    className={`flex flex-col items-center justify-center gap-1 px-4 py-3 touch-feedback transition-all duration-200
      ${active ? 'text-brand-blue' : 'text-text-sub'}`}
  >
...
export default function MainLayout() {
  const { user, logout } = useAuth();          // ← real user from JWT
  ...
  // Role-based nav: automatician and admin see all; viewer sees less
  ...
  return (
    <div className="flex min-h-screen bg-dashboard-bg overflow-x-hidden">
      ...
      <aside className="hidden lg:flex w-40 bg-[#17203f] border-r border-slate-100 flex-col items-start py-10 sticky lg:fixed lg:left-0 lg:top-0 h-screen z-20">
        ...
        {/* User badge */}
        <div className="px-4 mt-2 mb-2 w-full">
          <p className="text-xs text-gray-400 truncate">
            👤 <span className="font-semibold text-white">{user?.username}</span>
          </p>
          <p className="text-[10px] text-purple-400 font-medium capitalize">{user?.role}</p>
        </div>
        ...
        <div className="mt-auto w-full pb-6">
          <button
            ...
            className="flex items-center px-4 py-3 w-full rounded-lg text-gray-300 hover:bg-white/5 hover:text-white transition-all duration-300"
          >
            <span className="text-xl text-center font-medium">Logout</span>
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col w-full max-w-[1600px] mx-auto min-w-0 lg:ml-40 h-screen">
          ...
          <div className="bg-white p-2 rounded-panel shadow-soft-ui flex items-center gap-4 border border-slate-50">
            <div className="relative border-0 p-0">
              ...
              <input
                ...
                className="bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded-full border-0 outline-none placeholder:text-[12px] text-[12px] pl-10 pr-4 py-2 text-sm w-48 lg:w-[300px] lg:h-[33px] transition-all"
              />
            </div>
            {/* User avatar — shows real username on hover */}
            <div
              title={`${user?.username} (${user?.role})`}
              className="w-10 h-10 rounded-card bg-brand-blue overflow-hidden cursor-pointer shadow-soft-ui hover:shadow-fintech transition-shadow flex items-center justify-center"
            >
        ...
        <header className="flex md:hidden justify-between items-center px-4 py-4 bg-white border-b border-slate-100 z-30 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setSearchOpen(!searchOpen)} className="p-2 hover:bg-slate-50 rounded-card transition-colors touch-feedback">
              <Search size={20} className="text-text-main" />
            </button>
            <div className="w-8 h-8 rounded-card bg-brand-blue overflow-hidden cursor-pointer">
              <img src={`https://ui-avatars.com/api/?name=${user?.username || 'U'}&background=2D5BFF&color=fff&size=32`} alt="User" className="w-full h-full" />
            </div>
          </div>
        </header>
      ...
      <nav className="fixed bottom-0 left-0 right-0 md:hidden bg-white border-t border-slate-100 z-40" style={{ boxShadow: '0px 18px 40px rgba(112, 144, 176, 0.12)' }}>
        <div className="flex justify-around items-center h-16">
          <MobileNavItem icon={LayoutGrid} label="Dashboard" to="/mainlayout"          active={location.pathname === "/mainlayout"} />
          ...
          <MobileNavItem icon={Settings}   label="Logout"    to="/"
            active={false}
          />
        </div>
      </nav>
